		...
		$(document).ready(function() { 
			...
			$(".design").click(function() { 
                if (!$(this).hasClass("not")) { 
                    let expression = $("#expression").val().trim();

                    // Prevent duplicate operators
                    let lastChar = expression.slice(-1);
                    let currentChar = $(this).text();

                    if ("+-*/".includes(lastChar) && "+-*/".includes(currentChar)) {
                        return;
                    }

                    if (expression === "0" && !"+-*/".includes(currentChar)) {
                        $("#expression").val(currentChar);  
                    } else {
                        $("#expression").val(expression + currentChar);
                    }
                } 
            });
